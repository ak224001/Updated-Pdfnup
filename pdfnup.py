...
import base64
import types
from io import IOBase      ###### not called in python 2.7 pdfnup
from io import BytesIO   ######### not called in python 2.7 pdfnup
try:
    from StringIO import StringIO
except ImportError:
    from io import StringIO                #in pdfnup 2.7 it was----> from StringIO import StringIO


try:
    from PyPDF2 import PdfFileWriter, PdfFileReader         ###in python2.7 pdfnup it was ------> from pyPdf import PdfFileWriter, PdfFileReader
except ImportError:
    _MSG = "Please install PyPDF2 first, see http://pybrary.net/pyPdf"
    raise RuntimeError(_MSG)

from PyPDF2.pdf import PageObject, ContentStream         #from pyPdf.pdf import ImmutableSet---->from pyPdf import PdfFileWriter, PdfFileReader
try:
    from PyPDF2.pdf import ImmutableSet              #from pyPdf.pdf import ImmutableSet --->from pyPdf.pdf import ImmutableSet
except ImportError:
...
fgMis+cW
"""
bytes_text = bytes(_mtA4PdfZip64, 'utf-8')           ######## it was not included in python 2.7
_mtA4Pdf = zlib.decompress(base64.decodebytes(bytes_text))
...
def exP1multiN(pdf, newPageSize, n):
    ...
    buf = BytesIO()                     #############int python2.7---->buf = StringIO()
    buf.write(pdf)
    ...
    buf2 = BytesIO()          #############int python2.7---->buf = StringIO()
    output.write(buf2)
    buf2.seek(0)
    return buf2
...
def isFileLike(obj):
    ...
    if isinstance(obj, IOBase):           #############int python2.7 is was ---->if type(obj) == types.FileType:        
        return True
...
def generateNup(inPathOrFile, n, outPathPatternOrFile=None, dirs="RD",
    ...
    if isFileLike(ipof):
        ...
        if oppof is None:
            ...
            outFile = oppof
        elif isinstance(oppof,str):                    #############int python2.7 is was ---->if type(obj) == types.FileType: 
            outPath = oppof
            outFile = open(outPath, "wb")
    if isinstance(ipof,str):                        #############int python2.7 is was ---->if type(obj) == types.FileType: 
        inFile = open(ipof, "rb")
        if isFileLike(oppof):
            outFile = oppof
        elif oppof is None or isinstance(oppof,str):              #############int python2.7 is was ---->if type(obj) == types.FileType: 
            if oppof is None:
    ...
    if isinstance(oppof,str):               #############int python2.7 is was ---->if type(obj) == types.FileType: 
